Public Class FormProveedores
    ...
    Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        ...
        Else
            Dim rpta As String = Proveedor.registrar(nombre, ruc, contacto1, contacto2, banco, cuentaCorriente, direccion, telf, telf_sec, email, web, Distrito, emailcontacto1, emailcontacto2, telfcontacto1, telfcontacto2, Banco1, Banco2, moneda1, moneda2, CuentaCorriente1, CuentaCorriente2, CCI1, CCI2)
    ...
    Private Sub btnModificar_Click(sender As Object, e As EventArgs) Handles btnModificar.Click
        ...
        Dim id As String = dgvProveedores.Item(0, row).Value.ToString
        Dim nombre As String = dgvProveedores.Item(1, row).Value.ToString
        Dim ruc As String = dgvProveedores.Item(2, row).Value.ToString
        Dim Distrito As String = dgvProveedores.Item(3, row).Value.ToString
        Dim direccion As String = dgvProveedores.Item(4, row).Value.ToString
        Dim telf As String = dgvProveedores.Item(5, row).Value.ToString
        Dim telf_sec As String = dgvProveedores.Item(6, row).Value.ToString
        Dim email As String = dgvProveedores.Item(7, row).Value.ToString
        Dim web As String = dgvProveedores.Item(8, row).Value.ToString
        Dim contacto1 As String = dgvProveedores.Item(9, row).Value.ToString
        Dim EmailContacto1 As String = dgvProveedores.Item(10, row).Value.ToString
        Dim TelefonoContacto1 As String = dgvProveedores.Item(11, row).Value.ToString
        Dim contacto2 As String = dgvProveedores.Item(12, row).Value.ToString
        Dim EmailContacto2 As String = dgvProveedores.Item(13, row).Value.ToString
        Dim TelefonoContacto2 As String = dgvProveedores.Item(14, row).Value.ToString
        Dim banco1 As String = dgvProveedores.Item(15, row).Value.ToString
        Dim moneda1 As String = dgvProveedores.Item(16, row).Value.ToString
        Dim cuentaCorriente1 As String = dgvProveedores.Item(17, row).Value.ToString
        Dim CCI1 As String = dgvProveedores.Item(18, row).Value.ToString
        Dim banco2 As String = dgvProveedores.Item(19, row).Value.ToString
        Dim moneda2 As String = dgvProveedores.Item(20, row).Value.ToString
        Dim cuentaCorriente2 As String = dgvProveedores.Item(21, row).Value.ToString
        Dim CCI2 As String = dgvProveedores.Item(22, row).Value.ToString

        Dim formModificar As New ModifProveedor(id, nombre, ruc, Distrito, direccion, telf, telf_sec, email, web, contacto1, EmailContacto1, TelefonoContacto1, contacto2, EmailContacto2, TelefonoContacto2, banco1, moneda1, cuentaCorriente1, CCI1, banco2, moneda2, cuentaCorriente2, CCI2)
    ...
    Private Sub txtNombre_TextChanged(sender As Object, e As EventArgs)
        If txtNombre.Text.Length < 4 Then
            ErrorProveedor.SetError(txtNombre, "Ingrese al menos 4 caracteres")
        Else
            ErrorProveedor.SetError(txtNombre, "")
        End If
    End Sub

    Private Sub txtTelf_TextChanged(sender As Object, e As EventArgs)
        If txtTelf.Text.Length < 6 Then
            ErrorProveedor.SetError(txtTelf, "Ingrese un teléfono válido")
        Else
            ErrorProveedor.SetError(txtTelf, "")
        End If
    End Sub


    Public Function contieneArroba(ByVal cadena As String) As Boolean
        Return cadena.IndexOf("@") <> -1
    End Function

    Private Sub txtEmail_MouseLeave(sender As Object, e As EventArgs)
        If Not contieneArroba(txtEmail.Text) Then
            ErrorProveedor.SetError(txtEmail, "Se requiere ingresar un @")
        Else
            ErrorProveedor.SetError(txtEmail, "")
        End If
    End Sub
    ...
    Private Sub btnEliminar_Click(sender As Object, e As EventArgs) Handles btnEliminar.Click
        ...
        If result = DialogResult.Yes Then
            Dim id As String = dgvProveedores.Item(0, row).Value.ToString
            Dim rpta As String = Proveedor.eliminar(id)
            MessageBox.Show(rpta, "Respuesta", MessageBoxButtons.OK, MessageBoxIcon.Information)
            actualizarTabla()
        End If
    End Sub
    ...
    Private Function hayErrores() As Boolean
        If (ErrorProveedor.GetError(txtNombre) = Nothing And
            ErrorProveedor.GetError(txtTelf) = Nothing And
            ErrorProveedor.GetError(txtEmail) = Nothing And
            ErrorProveedor.GetError(txtDireccion) = Nothing And
            ErrorProveedor.GetError(txtTelfSec) = Nothing And
            ErrorProveedor.GetError(txtWeb) = Nothing) Then
            Return False
        End If
        Return True
    End Function
    ...
    Private Sub validarCampos(ByVal nivel As Integer)
        If txtNombre.Text.Trim.Length < 4 And nivel >= 1 Then
            ErrorProveedor.SetError(txtNombre, "Debe ingresar al menos 4 caracteres.")
        ElseIf (nivel = 1) Then
            ErrorProveedor.SetError(txtNombre, Nothing)
        End If

        If (txtTelf.Text.Length < 5 Or txtTelf.Text.Length = 0) And nivel >= 2 Then
            ErrorProveedor.SetError(txtTelf, "El teléfono debe tener al menos 5 caracteres.")
        ElseIf (nivel = 2) Then
            ErrorProveedor.SetError(txtTelf, Nothing)
        End If

        'If (Not HayArroba(txtEmail.Text)) And txtEmail.Text.Trim.Length > 0 And nivel >= 3 Then
        'ErrorProveedor.SetError(txtEmail, "Formato incorrecto de email (se requiere @).")
        'ElseIf (nivel = 3) Then
        'ErrorProveedor.SetError(txtEmail, Nothing)
        'End If

        If txtDireccion.Text.Trim.Length < 4 And nivel >= 4 Then
            ErrorProveedor.SetError(txtDireccion, "Debe ingresar al menos 4 caracteres.")
        ElseIf (nivel = 4) Then
            ErrorProveedor.SetError(txtDireccion, Nothing)
        End If

        If txtTelfSec.Text.Trim.Length < 5 And txtTelfSec.Text.Trim.Length > 0 And nivel >= 5 Then
            ErrorProveedor.SetError(txtTelfSec, "Debe ingresar al menos 5 caracteres.")
        ElseIf (nivel = 5) Then
            ErrorProveedor.SetError(txtTelfSec, Nothing)
        End If

        If txtWeb.Text.Trim.Length < 5 And txtWeb.Text.Trim.Length > 0 And nivel >= 6 Then
            ErrorProveedor.SetError(txtWeb, "Debe ingresar al menos 5 caracteres.")
        ElseIf (nivel = 6) Then
            ErrorProveedor.SetError(txtWeb, Nothing)
        End If
    ...
    Private Sub btnMaterialesAsociados_Click(sender As Object, e As EventArgs) Handles btnMaterialesAsociados.Click
        Dim row As Integer
        ...
        Dim id As String = dgvProveedores.Item(0, row).Value.ToString
        Dim nombre As String = dgvProveedores.Item(1, row).Value.ToString
        Dim formMaterialesAsociados As New FormMaterialesAsociados(id, nombre)
        formMaterialesAsociados.ShowDialog()
