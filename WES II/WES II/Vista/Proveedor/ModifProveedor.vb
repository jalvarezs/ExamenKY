Public Class ModifProveedor
    ...
    Private Sub ModifProveedor_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        ...
        Else
            Dim rpta As String = Proveedor.modificar(id, nombre, ruc, contacto1, contacto2, banco, cuentaCorriente, direccion, telf, telf_sec, email, web, Distrito, emailcontacto1, emailcontacto2, telfcontacto1, telfcontacto2, Banco1, Banco2, moneda1, moneda2, CuentaCorriente1, CuentaCorriente2, CCI1, CCI2)
    ...
    Private Sub txtNombre_TextChanged(sender As Object, e As EventArgs)
        If txtNombre.Text.Length < 4 Then
            ErrorModifProveedor.SetError(txtNombre, "El nombre debe tener mas de 4 caracteres")
        Else
            ErrorModifProveedor.SetError(txtNombre, "")
        End If
    End Sub

    Private Sub txtTelf_TextChanged(sender As Object, e As EventArgs)
        If txtTelf.Text.Length > 15 Then
            ErrorModifProveedor.SetError(txtTelf, "El telefono debe tener menos de 15 carcateres")
        Else
            ErrorModifProveedor.SetError(txtTelf, "")
        End If
    End Sub
    ...
    Private Sub txtEmail_MouseLeave(sender As Object, e As EventArgs)
        If Not HayArroba(txtEmail.Text) Then
            ErrorModifProveedor.SetError(txtEmail, "Formato incorrecto (miEmail@ccc.cc)")
        Else
            ErrorModifProveedor.SetError(txtEmail, "")
        End If
    End Sub

    Private Sub validarCampos(ByVal nivel As Integer)
        If txtNombre.Text.Trim.Length < 4 And nivel >= 1 Then
            ErrorModifProveedor.SetError(txtNombre, "Debe ingresar al menos 4 caracteres.")
        ElseIf (nivel = 1) Then
            ErrorModifProveedor.SetError(txtNombre, Nothing)
        End If

        If (txtTelf.Text.Length < 5 Or txtTelf.Text.Length = 0) And nivel >= 2 Then
            ErrorModifProveedor.SetError(txtTelf, "El teléfono debe tener al menos 5 caracteres.")
        ElseIf (nivel = 2) Then
            ErrorModifProveedor.SetError(txtTelf, Nothing)
        End If

        If (Not HayArroba(txtEmail.Text)) And txtEmail.Text.Trim.Length > 0 And nivel >= 3 Then
            ErrorModifProveedor.SetError(txtEmail, "Formato incorrecto de email (se requiere @).")
        ElseIf (nivel = 3) Then
            ErrorModifProveedor.SetError(txtEmail, Nothing)
        End If

        If txtDireccion.Text.Trim.Length < 4 And nivel >= 4 Then
            ErrorModifProveedor.SetError(txtDireccion, "Debe ingresar al menos 4 caracteres.")
        ElseIf (nivel = 4) Then
            ErrorModifProveedor.SetError(txtDireccion, Nothing)
        End If

        If txtTelfSec.Text.Trim.Length < 5 And txtTelfSec.Text.Trim.Length > 0 And nivel >= 5 Then
            ErrorModifProveedor.SetError(txtTelfSec, "Debe ingresar al menos 5 caracteres.")
        ElseIf (nivel = 5) Then
            ErrorModifProveedor.SetError(txtTelfSec, Nothing)
        End If

        If txtWeb.Text.Trim.Length < 5 And txtWeb.Text.Trim.Length > 0 And nivel >= 6 Then
            ErrorModifProveedor.SetError(txtWeb, "Debe ingresar al menos 5 caracteres.")
        ElseIf (nivel = 6) Then
            ErrorModifProveedor.SetError(txtWeb, Nothing)
        End If

    End Sub

    Private Function hayErrores() As Boolean
        If (ErrorModifProveedor.GetError(txtNombre) = Nothing And
            ErrorModifProveedor.GetError(txtTelf) = Nothing And
            ErrorModifProveedor.GetError(txtEmail) = Nothing And
            ErrorModifProveedor.GetError(txtDireccion) = Nothing And
            ErrorModifProveedor.GetError(txtTelfSec) = Nothing And
            ErrorModifProveedor.GetError(txtWeb) = Nothing) Then
            Return False
        End If
        Return True
    End Function
